...
async function carregarProdutos() {
  try {
    ...
    if (res.sucesso && Array.isArray(res.dados)) {
      res.dados.forEach(p => {
        // 🔧 Normaliza possíveis variações de chave no retorno
        const nome =
          p.nome || p.nome_produto || p.produto_nome || "[Sem nome]";
        select.insertAdjacentHTML(
          "beforeend",
          `<option value="${p.id}">${nome}</option>`
        );
      });
    }
  } catch (err) {
...
async function carregarRelatorio(pagina = 1) {
  ...
  try {
    ...
    const resp = await apiFetch("api/relatorios.php?" + query);
    const res = await resp.json();
...
document.getElementById("btn-limpar")?.addEventListener("click", () => {
  ...
  document.querySelector("#tabelaRelatorios tbody").innerHTML = `<tr><td colspan="6" class="text-center text-muted">Use os filtros para buscar movimentações</td></tr>`;
  atualizarGraficos({});
  atualizarPaginacao(1, 1);
});
